...
```tsx
import { requestModeSchema, requestSchema } from "zod-request";
...
```typescript
const requestModeEnumSchema: z.ZodEnum<["same-origin", "no-cors", "cors", "navigate"]>
```
...
```tsx
import { requestModeEnumSchema, requestSchema } from "zod-request";
