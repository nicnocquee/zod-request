...
<Callout type="info">
  The protocol is automatically extracted from the request URL. The URL protocol
  includes a colon (e.g., "https:"), but the validated protocol value is
  returned without the colon (e.g., "https"). This matches the standard web
  protocol format.
</Callout>
